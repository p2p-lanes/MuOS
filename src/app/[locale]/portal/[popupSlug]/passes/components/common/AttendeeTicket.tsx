            ...
            <div className="z-10 h-full flex xl:flex-col justify-between xl:justify-start xl:gap-10">
              <div className="flex flex-col justify-center xl:order-2">
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <p className="text-sm text-gray-500">{t(`badgeNames.${attendee.category}` as Parameters<typeof t>[0]) || attendee.category}</p>
                </div>
              </div>
          ...
          <div className="flex flex-col p-8 gap-2 xl:pr-10">
            {standardProducts.length === 0 ? (
              <div className="flex w-full h-full justify-center items-center">
                <p className="text-sm font-medium text-neutral-500">{t('attendeeTicket.comingSoon')}</p>
              </div>
              ...
              <div className="flex w-full h-full justify-center items-center p-4">
                <p className="text-sm font-medium text-neutral-500 text-center">
                  ...
                  >
                    {t('attendeeTicket.goToBuyPasses')}
                  </span>{" "}
                  {t('attendeeTicket.toPurchase')}
                </p>
              </div>
            ) : (
                ...
                {localProducts.length > 0 && (
                  <Collapsible open={localOpen} onOpenChange={setLocalOpen} className="space-y-2">
                    <CollapsibleTrigger className="w-full bg-accent rounded-md" aria-label="Toggle Local Tickets">
                      <div className="flex justify-between items-center p-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium">{t('attendeeTicket.latamCitizens')}</span>
                        </div>
                      </div>
                    </CollapsibleTrigger>
                    <CollapsibleContent className="transition-all duration-100 ease-in-out data-[state=closed]:animate-slideUp data-[state=open]:animate-slideDown">
                      ...
                      <p className="text-sm font-medium text-neutral-500 text-right mt-2">{t('attendeeTicket.idRequired')}</p>
                    ...
                    <CollapsibleTrigger className="w-full bg-accent rounded-md" aria-label="Toggle Common Tickets">
                      <div className="flex justify-between items-center p-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium">{t('attendeeTicket.standard')}</span>
                        </div>
                      </div>
                    </CollapsibleTrigger>
                    <CollapsibleContent className="transition-all duration-100 ease-in-out data-[state=closed]:animate-slideUp data-[state=open]:animate-slideDown">
...

            {
              hasPurchased && (
                <div className="flex w-full justify-end">
                  ...
                  >
                    <p className="text-sm font-medium">{t('attendeeTicket.checkInCode')}</p>
