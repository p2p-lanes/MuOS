...
const MemberFormModal = ({ open, onClose, onSuccess, member }: MemberFormModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditMode && member) {
        ...
        await api.put(`/groups/${group_id}/members/${member.id}`, processedData).then((res) => {
          if (res.status === 200) {
            ...
            toast.error(t('updateMemberFailed'))
          }
        })
      } else {
        ...
        await api.post(`/groups/${group_id}/new_member`, processedData).then((res) => {
          if (res.status === 200) {
            ...
            console.log('ress', res)
            toast.error(res.data.detail || t('addMemberFailed'))
          }
        })
      }
      ...
      toast.error(error.response?.data?.message || (isEditMode ? t('updateMemberFailed') : t('addMemberFailed')))
    } finally {
